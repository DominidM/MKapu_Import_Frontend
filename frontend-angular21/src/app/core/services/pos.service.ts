...
export class PosService {
  private readonly MEDIOS: Record<
    string,
    {
      label: string;
      icon: string;
  ...
  requiereBanco(medio: string): boolean {
    return ['TARJETA_DEBITO', 'TARJETA_CREDITO', 'DEPOSITO_CUENTA', 'TARJETA'].includes(
      this.normalizar(medio),
    );
  }
  ...
  validarPago(pago: Partial<Pago>): { valido: boolean; errores: string[] } {
    ...
    if (this.requiereBanco(pago.med_pago!) && !pago.banco)
  ...
  registrarPago(pago: Omit<Pago, 'id_pago'>): Pago {
    const nuevo: Pago = {
      ...pago,
      id_pago: this.pagos.length > 0 ? Math.max(...this.pagos.map((p) => p.id_pago)) + 1 : 1,
      fec_pago: new Date(),
    };

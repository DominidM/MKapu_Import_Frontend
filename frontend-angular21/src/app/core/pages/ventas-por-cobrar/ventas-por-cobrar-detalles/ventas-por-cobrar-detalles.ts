...
export class VentasPorCobrarDetalles implements OnInit {
  ...
  constructor(
    ...
    private productoService: Produ  ctoService
  ) {}
  ...
  eliminarProducto(event: Event) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        const exito = this.productoService.eliminarProducto(this.producto!.id!);
        if (exito) {
          this.messageService.add({
            severity: 'success',
  ...
  restaurarProducto(event: Event) {
    ...
    this.confirmationService.confirm({
      ...
      rejectButtonProps: { severity: 'secondary', outlined: true },
      accept: () => {
        const exito = this.productoService.restaurarProducto(this.producto!.id!);
        if (exito) {
          this.messageService.add({
            severity: 'success',
  ...
  get margenGanancia(): number {
    ...
    return this.productoService.getMargenGanancia(this.producto);
  }

  get porcentajeMargen(): number {
    if (!this.producto) return 0;
    return this.productoService.getPorcentajeMargen(this.producto);
  }

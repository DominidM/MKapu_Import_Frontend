...
export class FormularioCliente implements OnInit {
  ...
  get nombresValido(): boolean {
    return this.esRUC || String(this.cliente.nombres ?? '').trim().length >= 2;
  }
  get apellidosValido(): boolean {
    return this.esRUC || String(this.cliente.apellidos ?? '').trim().length >= 2;
  }
  get razonSocialValida(): boolean {
    return !this.esRUC || String(this.cliente.razonsocial ?? '').trim().length >= 3;
  }
    ...
    this.clienteService.createCustomer(payload).subscribe({
      next: (created) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Registrado',
          detail: `Cliente ${created.displayName ?? created.documentValue} creado.`,
          life: 3000,
        });

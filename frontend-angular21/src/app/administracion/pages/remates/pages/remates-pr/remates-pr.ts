...
export class RematesPr implements OnInit {
  ...
  productosFiltrados = computed(() => {
    ...
    return this.rematesPorEstado().filter(
      (r) =>
  ...
  private mapToUI(a: AuctionResponseDto): RemateUI {
    ...
    const descuento =
      precioOriginal > 0 ? Math.round(((precioOriginal - precioRemate) / precioOriginal) * 100) : 0;
  ...
  private getNombreSede(id_sede: number): string {
    if (!id_sede) return '—';
    return (
      this.sedeService.sedes().find((s) => s.id_sede === id_sede)?.nombre ?? `Sede #${id_sede}`
    );
  }
  ...
  limpiarFiltros(): void {
    ...
    this.messageService.add({
      ...
      detail: 'Se muestran todos los remates',
      life: 2000,
    });
  }
    ...
    this.auctionService.finalizeAuction(remate.id_remate).subscribe({
      ...
      error: () =>
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'No se pudo cambiar el estado del remate.',
          life: 3000,
        }),
    });
  }

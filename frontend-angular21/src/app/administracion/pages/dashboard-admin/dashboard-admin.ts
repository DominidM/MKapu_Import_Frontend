...
export class DashboardAdmin implements OnInit {
  ...
  getNombreSede(): string {
    ...
    const id    = this.idSede();
    if (!id || !sedes.length) return 'Mi sede';
    const sede  = sedes.find((s) => String(s.id_sede) === String(id));
    return sede?.nombre ?? 'Mi sede';
  }
  ...
  getAbs(value: number): number { return Math.abs(value); }
  ...
  cargarGraficoVentasPorCategoria(): void {
    this.dashboardService
      ...
      .subscribe({
        next: (data) => {
          if (!data?.labels?.length) { this.ventasPorCategoriaChart.set(null); return; }
        ...
        error: (err) => { console.error('Error categorías', err); this.ventasPorCategoriaChart.set(null); },
      });
  }
  ...
  cargarGraficoMetodosPago(): void {
    this.dashboardService
      ...
      .subscribe({
        next: (data) => {
          if (!data?.labels?.length) { this.metodosPagoChart.set(null); return; }
        ...
        error: (err) => { console.error('Error métodos de pago', err); this.metodosPagoChart.set(null); },
      });
  }
  ...
  cargarGraficoVentasPorSede(): void {
    this.dashboardService
      ...
      .subscribe({
        next: (data) => {
          if (!data?.labels?.length) { this.ventasPorSedeChart.set(null); return; }
          const sedes  = this.sedesOptions();
        ...
        error: (err) => { console.error('Error ventas por sede', err); this.ventasPorSedeChart.set(null); },
      });
  }
  ...
  cargarGraficoVentasPorDistrito(): void {
    this.dashboardService
      ...
      .subscribe({
        next: (data) => {
          if (!data?.labels?.length) { this.ventasPorDistritoChart.set(null); return; }
        ...
        error: (err) => { console.error('Error ventas por distrito', err); this.ventasPorDistritoChart.set(null); },
      });
  }
        ...
        next: (data: any[]) => {
          const actividadMapeada: ActividadReciente[] = data.map((c) => {
            ...
            const tiempo =
              ...
              minutos < 1440  ? `Hace ${Math.floor(minutos / 60)} hrs` :
  ...
  configurarOpcionesGraficos(): void {
    ...
    this.chartOptions = {
      ...
      plugins: {
        ...
        tooltip: {
          ...tooltipBase,
          callbacks: { label: (ctx: any) => `S/ ${ctx.parsed.y.toLocaleString('es-PE', { minimumFractionDigits: 0 })}` },
        },
      },
      scales: {
        x: { ticks: { color: textColorSecondary, font: { size: 11 } }, grid: { color: 'transparent' } },
        y: { ticks: { color: textColorSecondary, callback: (v: any) => 'S/ ' + (v / 1000).toFixed(0) + 'k' }, grid: { color: surfaceBorder } },
      },
    };
    ...
    this.barChartOptions = {
      ...
      plugins: {
        ...
        tooltip: {
          ...tooltipBase,
          callbacks: { label: (ctx: any) => `S/ ${ctx.parsed.x.toLocaleString('es-PE', { minimumFractionDigits: 0 })}` },
        },
      },
      scales: {
        x: { ticks: { color: textColorSecondary, callback: (v: any) => 'S/ ' + (v / 1000).toFixed(0) + 'k' }, grid: { color: surfaceBorder } },
        y: { ticks: { color: textColorSecondary, font: { size: 11 } }, grid: { color: 'transparent' } },
      },
    };
    ...
    this.doughnutChartOptions = {
      ...
      plugins: {
        legend: {
          ...
          position: 'bottom',
          align: 'center',
          labels: { color: textColor, font: { size: 11 }, padding: 10, usePointStyle: true, pointStyleWidth: 8, boxWidth: 8 },
        },
        tooltip: {
          ...tooltipBase,
          callbacks: {

...
export class Sedes implements OnInit {
  ...
  ngOnInit(): void {
    // Carga sedes y luego todos sus almacenes en paralelo
    this.sedeService.loadSedes('Administrador').pipe(
      switchMap((res) => {
        const sedes = res.headquarters ?? [];
        if (sedes.length === 0) return of([]);

        const requests$ = sedes.map((sede) =>
          this.sedeService.loadAlmacenesParaSede(sede.id_sede).pipe(
            catchError(() => of(null)) // si una falla, continúa con las demás
          )
        );
  ...
  verDetalle(sede: Headquarter): void {
    // Busca la sede enriquecida desde la señal (ya tiene almacenes cargados)
    const enriquecida = this.sedeService.sedes()
      .find((s) => s.id_sede === sede.id_sede) ?? sede;

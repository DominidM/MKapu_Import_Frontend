...
export class AuctionService {
    ...
    return this.getAuctionById(id, role).pipe(
      switchMap((actual) => {
        ...
        const detallesPayload =
          detallesCambiaron && dto.detalles
        ...
        return this.http
          .put<AuctionResponseDto>(`${this.api}/logistics/auctions/${id}`, payload, {
            headers: this.buildHeaders(role),
          })
          .pipe(
            tap((updated) =>
              this._auctions.update((list) =>
                list.map((a) => (a.id_remate === updated.id_remate ? updated : a)),
              ),
            ),
          );
      }),
      ...
      .pipe(
        tap((response) => {
          ...
          this._totalPages.set(
            Math.max(1, Math.ceil((response.total ?? response.items?.length ?? 0) / (limit || 1))),
          );
  ...
  finalizeAuction(id: number, role = 'Administrador'): Observable<AuctionResponseDto> {
    ...
    this._error.set(null);
    return this.http
      .post<AuctionResponseDto>(
        `${this.api}/logistics/auctions/${id}/finalize`,
        {},
        { headers: this.buildHeaders(role) },
      )
      .pipe(
